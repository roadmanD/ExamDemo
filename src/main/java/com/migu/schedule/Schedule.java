...
//        List<TaskInfo> sumTaskInfoList = getSumTaskInfoList();
        ...
        //把账单按从大到小排序
        Collections.sort(waitTaskInfoList,new Comparator<TaskInfo>() {
            @Override
...
//        }
        for(Integer key : nodeTaskMap.keySet()){
            nodeTaskTestMap.put(key,nodeTaskMap.get(key) == null ? new ArrayList<TaskInfo>() : nodeTaskMap.get(key));
        }
        for(int i = waitTaskInfoList.size() - 1;i >= 0; i--){
            Integer nodeId = getMixComNodeId();
            List<TaskInfo> tList = nodeTaskTestMap.get(nodeId) == null ? new ArrayList<TaskInfo>() : nodeTaskTestMap.get(nodeId);

            TaskInfo tt = waitTaskInfoList.get(i);
            tt.setNodeId(nodeId);
            tList.add(tt);
